...
export function QueueView() {
  ...
  const [offlineItems, setOfflineItems] = useState<Array<{ id: string; content: string; timestamp: number }>>([])

  const handleRefresh = useCallback(() => {
    fetchQueue()
    getOfflineQueue().then(items => {
      setOfflineItems(items.map(i => ({
        id: i.id,
    ...
    <div className="q-body">
            ...
            {pendingJobs.map((job, index) => (
              ...
              >
                ...
                <div className="qi-body">
                  <div className="qi-title">{truncateContent(job.note_path || job.type)}</div>
                  <div className="qi-meta">{job.type} · {job.status}</div>
                </div>
                <div className={`qi-dot ${job.status === 'queued' ? 'q' : 'p'}`} />
      ...
      {failedJobs.length > 0 && (
        <>
          ...
          {failedJobs.length > 1 && (
            <RetryAllBanner count={failedJobs.length} onRetryAll={handleRetryAll} />
          )}
          <div className="q-list">
            {failedJobs.map((job, index) => (
              ...
              >
                {index === 0 ? (
                  <FailedJobExpanded job={job} onRetry={handleRetry} onDiscard={handleDiscard} />
                ) : (
                  <FailedJobCard job={job} onRetry={handleRetry} onDiscard={handleDiscard} />
                )}
              </motion.div>
            ))}
          </div>
      ...
      {doneJobs.length > 0 && (
        <>
          ...
          {doneJobs.length > 5 && (
            <div className="done-expand">{doneJobs.length - 5} more · scroll to expand</div>
          )}
        </>
      )}
